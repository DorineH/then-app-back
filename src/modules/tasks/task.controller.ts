...
import { ApiBearerAuth, ApiOkResponse, ApiTags, ApiCreatedResponse, ApiNoContentResponse } from '@nestjs/swagger';
import { CreateTaskDto, UpdateTaskDto, DayQueryDto, RangeQueryDto } from './dto/task.dto';
...
@ApiTags('tasks')
@ApiBearerAuth()
@UseGuards(JwtAuthGuard)@Controller('tasks')
...
async getByDay(@Req() req: any, @Query() query: DayQueryDto): Promise<Task[]> {
...
async getRange(@Req() req: any, @Query() query: RangeQueryDto): Promise<Task[]> {
const user = req.user as AuthUser;
const list = await this.service.findRange(user.coupleId, query.from, query.to);
return list.map(mapTask);
}
...
const user = req.user as AuthUser;
const created = await this.service.create(user.sub, user.coupleId, dto);
...
async update(@Req() req: any, @Param('id') id: string, @Body() dto: UpdateTaskDto): Promise<Task> {
const user = req.user as AuthUser;
const updated = await this.service.update(id, user.sub, user.coupleId, dto);
...
const user = req.user as AuthUser;
await this.service.remove(id, user.sub, user.coupleId);
}
}